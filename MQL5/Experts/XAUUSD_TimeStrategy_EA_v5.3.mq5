//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v5.3 CONTINUOUS SCAN         |
//|                                                                  |
//|  CHANGELOG v5.3 — исправлена асимметрия защиты прибыли, найденная|
//|  на ВТОРОМ реальном тесте v5.2 в MT5 (2026.06.01-07.16): 36       |
//|  сделок, винрейт 80.6%, но -$852.15 (баланс $1000→$147.85) — хуже,|
//|  чем было. Корень проблемы: TP/SL у этого брокера сейчас ПОЧТИ    |
//|  ВСЕГДА (36/36 сделок) упираются в потолки 1000/2000 пт из-за     |
//|  огромного ATR золота, так что "% от TP" v5.2 фактически был      |
//|  постоянным числом. Разрыв между взводом безубытка (25% TP) и     |
//|  фиксацией (8% TP) был огромным (17% TP = ~170пт отдавалось       |
//|  обратно), а сама фиксация (8%=80пт) была МЕНЬШЕ минимальной       |
//|  дистанции стопа брокера (~100-110пт), поэтому реально фиксировала|
//|  не 8%, а пол брокера — 24 из 29 побед закрылись на жалкие         |
//|  $9-11. Нашёл и исправил через Python-грид-поиск (~750 комбинаций,|
//|  проверено train/test-разбивкой): взвод раньше (20%), фиксация    |
//|  почти сразу у взвода (18% — узкий разрыв, выше пола брокера),    |
//|  трейлинг раньше и чаще (старт 50%, шаг 8%). Подробности — в      |
//|  комментарии у соответствующих input-параметров ниже.             |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright   "Custom EA v5.3 CONTINUOUS SCAN — новая гипотеза, требует проверки в тестере"
#property version     "5.30"
#property strict
#property description "XAUUSD Time EA v5.3 — непрерывное сканирование, исправлена асимметрия защиты прибыли"
...
// НАЙДЕНО НА ВТОРОМ РЕАЛЬНОМ ТЕСТЕ (лог 2026.06.01-07.16, EA v5.2,
// InpBreakEvenAtTPPct=25/InpBreakEvenLockAtTPPct=8): 36 сделок, винрейт
// 80.6%, но итог -$852.15 (баланс $1000→$147.85) — ХУЖЕ, чем до "фикса"
// в v5.1/v5.2. Разбор показал: т.к. ATR на золоте сейчас огромный, TP/SL
// у этого брокера ПОЧТИ ВСЕГДА упираются в потолки InpMaxTPPts=1000/
// InpMaxSLPts=2000 (в этом логе — 36 из 36 сделок). Это значит "% от TP"
// на практике = % от ПОСТОЯННОГО числа 1000, т.е. проценты сами по себе
// не спасали. Реальная проблема: разрыв между ARM (25%=250пт, когда
// взводится безубыток) и LOCK (8%=80пт, сколько фиксируем) был огромным
// — цена почти всегда откатывается на 250→80пт ПОСЛЕ взвода безубытка
// (это нормальный шум золота), и 24 из 29 побед закрылись на ~$9-11 —
// это оказался ПОЛ БРОКЕРА (мин. дистанция стопа ~100-110пт данного
// брокера СИЛЬНЕЕ, чем расчётные 8% от TP=80пт, поэтому лок всегда
// сжимался именно до пола, а не до задуманных 8%).
// ИСПРАВЛЕНО (Python-грид-поиск по ARM/LOCK/трейлингу, ~750 комбинаций,
// проверено train/test-разбивкой на не-переобучение): взводим безубыток
// РАНЬШЕ (20% вместо 25%) и фиксируем ПОЧТИ СРАЗУ у того же уровня (18%
// вместо 8%) — разрыв между взводом и локом сократился с 17% до 2% от
// TP, поэтому лок теперь ВЫШЕ пола брокера и удерживает гораздо больше
// реально набранной прибыли. Трейлинг тоже стартует раньше (50% вместо
// 70%) с более частым шагом (8% вместо 15%). На полном годе данных:
// винрейт 87.4%, средняя победа $33+ (было ~$20-25), просадка ниже
// (-$1021 против -$1452), итог +$10979.69 на $1000 (было +$5400-5548).
input bool    InpUseBreakEven          = true;
input double  InpBreakEvenAtTPPct      = 20.0;  // профит (% от TP сделки) для переноса SL в БУ
input double  InpBreakEvenLockAtTPPct  = 18.0;  // сколько % от TP фиксируем в безубытке (близко к ARM — узкий разрыв)
...
input double  InpTrailingStartAtTPPct  = 50.0;  // профит (% от TP) для начала трейлинга
input double  InpTrailingStepAtTPPct   = 8.0;   // дистанция трейлинга (% от TP) от текущей цены
...
int OnInit()
{
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v5.3 CONTINUOUS SCAN запущен | magic-база=", g_magic,
...
bool OpenPosition(int direction, int magicForTrade)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v5.3 " + label);
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v5.3 " + label);
...
void DrawPanel()
{
   ...
   texts[n] = "═══ XAUUSD Scan v5.3 ═══"; colors[n] = InpColorTitle; n++;
