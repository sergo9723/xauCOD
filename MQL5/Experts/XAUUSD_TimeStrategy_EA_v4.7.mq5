//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v4.7 READABLE PANEL           |
//|                                                                  |
//|  CHANGELOG v4.7 — подготовка к запуску НЕСКОЛЬКИХ инстансов      |
//|  EA на разные сессии/окна времени (один и тот же файл, разные    |
//|  входные параметры на разных графиках):                          |
//|                                                                  |
//|  БАГ #F: magic-номер был захардкожен в коде (202507) — если      |
//|     запустить несколько инстансов этого EA (напр. на разные      |
//|     сессии 02:00-06:00 / 14:00-17:00 / 20:00-22:00), они все      |
//|     работали бы с ОДНИМ magic. Последствия: HasOpenShort/Long     |
//|     одного инстанса видели бы позиции ДРУГОГО инстанса и          |
//|     отказывались бы открывать свою (ложное "уже открыто");        |
//|     стоп-машина (счётчик убытков подряд) была бы общей на все    |
//|     инстансы вместо отдельной для каждого; персистентность        |
//|     стоп-машины (v4.6, ключ по symbol+magic) тоже пересекалась    |
//|     бы между инстансами.                                          |
//|     ИСПРАВЛЕНО: добавлен input InpMagicNumber — при запуске       |
//|     нескольких инстансов КАЖДОМУ на своём графике задайте          |
//|     РАЗНЫЙ magic в настройках EA.                                 |
//|                                                                  |
//|  Всё из v4.3/v4.4/v4.5/v4.6 сохранено без изменений: фильтр      |
//|  объёма (исправлен в v4.5), риск % от баланса, безубыток/         |
//|  частичное закрытие/трейлинг, проверка hedging-счёта, минуты-от- |
//|  полуночи для окон входа, опциональный H1-фильтр тренда,         |
//|  корректный учёт win/loss при частичных закрытиях, персистентная |
//|  стоп-машина (v4.6).                                              |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright   "Custom EA v4.7 READABLE PANEL — audited"
#property version     "4.70"
#property strict
#property description "XAUUSD Time EA v4.7 — magic number как input, для запуска нескольких сессий"
...
int OnInit()
{
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v4.7 READABLE PANEL запущен");
...
bool OpenPosition(int direction)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v4.7 LONG");
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v4.7 SHORT");
...
void DrawPanel()
{
   ...
   texts[n] = "═══ XAUUSD Reversal v4.7 ═══"; colors[n] = InpColorTitle; n++;
