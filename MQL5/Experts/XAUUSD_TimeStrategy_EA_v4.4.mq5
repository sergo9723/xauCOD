//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v4.4 READABLE PANEL           |
//|                                                                  |
//|  CHANGELOG v4.4 (второй аудит, фокус на исполнении ордеров,     |
//|  расстановке SL/TP и точности сигналов) — найденные баги:       |
//|                                                                  |
//|  БАГ #A (реальный, средней тяжести): окно входа считалось в     |
//|     псевдо-десятичном формате ЧЧММ (hour*100+minute), и граница |
//|     окна `entryTo = entryFrom + EntryWindow` ломается, если     |
//|     EntryMinute+EntryWindow > 59 — арифметика "перепрыгивает"   |
//|     через несуществующие "ЧЧ:60..99" и окно входа реально        |
//|     обрезается на несколько минут раньше, чем задумано.          |
//|     С дефолтными настройками (Minute=0, Window=5) бага не видно, |
//|     но при любой другой комбинации входа она "отъедает" минуты. |
//|     ИСПРАВЛЕНО: все временные ключи (анализ/вход, валидация,    |
//|     панель) переведены в "минуты от полуночи" (hour*60+minute) — |
//|     арифметика с окнами теперь всегда корректна.                  |
//|                                                                  |
//|  БАГ #B (реальный, но проявляется только при нестандартных      |
//|     значениях InpTrailingStartPts/StepPts): в ManageOpenPositions|
//|     безубыток, частичное закрытие и трейлинг в одном тике шли    |
//|     подряд, но трейлинг сравнивал новый SL с ЛОКАЛЬНОЙ переменной|
//|     curSL, прочитанной ДО того, как безубыток мог её изменить —  |
//|     т.е. в редких конфигурациях трейлинг мог "откатить" защиту,  |
//|     которую только что поставил безубыток. С дефолтными          |
//|     параметрами (BE 300/50, Trailing 500/150) это не проявляется,|
//|     но это скрытая мина при других значениях.                    |
//|     ИСПРАВЛЕНО: локальная curSL обновляется сразу после успешного|
//|     PositionModify, все последующие блоки в этом тике видят      |
//|     актуальное состояние.                                        |
//|                                                                  |
//|  СИГНАЛЫ: фильтры "движение цены" / "цвет свечи M5" / "цена      |
//|     выше/ниже EMA M15" сильно коррелируют между собой (все три   |
//|     по сути про "цена уже пошла в нужную сторону") — счёт 4/6    |
//|     легче набрать, чем кажется. Добавлен опциональный            |
//|     InpUseH1TrendFilter (выкл. по умолчанию): при включении      |
//|     фильтр EMA считается на H1 вместо M15 — независимое          |
//|     подтверждение со старшего таймфрейма вместо ещё одной        |
//|     M15-версии того же сигнала.                                  |
//|                                                                  |
//|  Всё из v4.3 (см. историю коммитов) сохранено без изменений:    |
//|  реальный фильтр объёма, риск % от баланса, безубыток/частичное  |
//|  закрытие/трейлинг, стоп-машина после серии убытков, проверка   |
//|  hedging-счёта, корректный учёт win/loss при частичных закрытиях.|
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright   "Custom EA v4.4 READABLE PANEL — audited"
#property version     "4.40"
#property strict
#property description "XAUUSD Time EA v4.4 — фикс окна входа + trailing race + H1 фильтр"
...
int OnInit()
{
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v4.4 READABLE PANEL запущен");
...
bool OpenPosition(int direction)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v4.4 LONG");
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v4.4 SHORT");
...
void DrawPanel()
{
   ...
   texts[n] = "═══ XAUUSD Reversal v4.4 ═══"; colors[n] = InpColorTitle; n++;
