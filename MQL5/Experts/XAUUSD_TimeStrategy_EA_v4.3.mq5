//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v4.3 READABLE PANEL           |
//|                                                                  |
//|  CHANGELOG v4.3 (аудит от v4.2) — исправления и улучшения:      |
//|                                                                  |
//|  БАГИ:                                                            |
//|  #1 InpVolPeriod был объявлен как input, но нигде не             |
//|     использовался — фильтр объёма реально не считался.          |
//|     Добавлена функция VolumeConfirmed(), заменившая              |
//|     дублирующую проверку спреда в 6-м фильтре (спред и так       |
//|     жёстко проверяется в SpreadOK() перед входом — держать      |
//|     его ещё и "бесплатным" баллом в счёте было ошибкой).        |
//|  #2 Риск-менеджмент по InpRiskPct был мёртвым кодом:             |
//|     CalcLotByRisk() вызывался только если InpLotSize<=0,        |
//|     а по умолчанию InpLotSize=0.01 — риск % никогда не          |
//|     применялся. Добавлен явный переключатель InpUseFixedLot.    |
//|  #3 Валидация времени анализ→вход сравнивала часы только когда  |
//|     они совпадают (InpXAnalysisHour==InpXEntryHour) — если      |
//|     часы разные, некорректная конфигурация (вход раньше         |
//|     анализа) проходила проверку. Теперь сравниваются полные     |
//|     ЧЧММ-ключи.                                                  |
//|  #4 Высота тёмного фона панели была захардкожена (totalLines=16)|
//|     а реальное число строк при всех условных полях доходило     |
//|     до ~20 — текст вылезал за фон. Теперь фон считается по      |
//|     фактическому числу строк.                                    |
//|  #5 Учёт win/loss ломался при частичном закрытии позиции        |
//|     (см. ниже, новая функция) — раньше такого сценария не было, |
//|     теперь это учтено через накопление PnL по position_id.      |
//|                                                                  |
//|  ГЛАВНАЯ ПРОБЛЕМА РИСК-МЕНЕДЖМЕНТА (см. отчёт в чате):          |
//|  TP=700 / SL=2500 → соотношение риск:прибыль ≈ 3.57:1.          |
//|  Чтобы просто не терять деньги, нужен винрейт ≥78%. Один минус  |
//|  "съедает" 3.5 плюса. Это и есть причина "3 плюса, 1 минус —    |
//|  а по факту почти в ноль/в минус".                               |
//|                                                                  |
//|  НОВОЕ (управление уже открытой позицией, сокращает средний     |
//|  убыток без изменения вашего TP/SL — они остаются как были):    |
//|  • Безубыток (breakeven): после +InpBreakEvenTriggerPts SL      |
//|    переносится в +InpBreakEvenLockPts — разворот после этого    |
//|    стоит не 2500 пт, а -50 пт.                                   |
//|  • Частичное закрытие: на InpPartialCloseAtTPPct% от TP         |
//|    фиксируется InpPartialClosePct% объёма.                       |
//|  • Трейлинг-стоп после безубытка — даёт прибыли расти дальше    |
//|    TP, если движение продолжается.                               |
//|  • Стоп-машина: после InpMaxConsecutiveLosses убытков подряд    |
//|    торговля приостанавливается до понедельника — не даёт        |
//|    "плохой серии" убить неделю.                                  |
//|  • Проверка hedging-счёта — стратегия требует одновременных     |
//|    SHORT+LONG, на netting-счёте это будет схлопываться.         |
//|  • Опциональные ATR-стопы (выключены по умолчанию) — SL/TP      |
//|    считаются от волатильности, а не фиксированными пунктами.    |
//|                                                                  |
//|  ЛОГИКА ВХОДОВ (без изменений от v4.2):                          |
//|  • 03:00-03:50 анализ SHORT → если ≥4/6 → 04:00 ВХОД           |
//|  • 15:00-15:50 анализ LONG → если ≥4/6 → 16:00 ВХОД            |
//|  • Обе позиции одновременно возможны (нужен hedging-счёт)       |
//+------------------------------------------------------------------+
#property copyright   "Custom EA v4.3 READABLE PANEL — audited"
#property version     "4.30"
#property strict
#property description "XAUUSD Time EA v4.3 — аудит рисков + управление позицией"
...
input int     InpTakeProfit       = 700;     // TP в пунктах
...
input group "=== ВРЕМЕНА АНАЛИЗА И ВХОДА ==="
input int     InpShortAnalysisHour    = 3;   // Анализ SHORT: с 03:00
input int     InpShortAnalysisMinute  = 0;
input int     InpShortAnalysisEndMin  = 50;  // до 03:50
input int     InpShortEntryHour       = 4;   // Вход SHORT: 04:00
input int     InpShortEntryMinute     = 0;
input int     InpShortEntryWindow     = 5;   // окно входа 5 мин

input int     InpLongAnalysisHour     = 15;  // Анализ LONG: с 15:00
input int     InpLongAnalysisMinute   = 0;
input int     InpLongAnalysisEndMin   = 50;  // до 15:50
input int     InpLongEntryHour        = 16;  // Вход LONG: 16:00
...
// FIX v4.3 #1: реальный фильтр объёма (раньше InpVolPeriod нигде не применялся)
bool VolumeConfirmed()
{
   ...
   long curVol = vol[need - 1];
   long sum = 0;
   for(int i = 0; i < need - 1; i++) sum += vol[i];
...
int CountShortFilters(double startPrice)
{
   ...
   // 6. FIX v4.3 #1: объём выше среднего (было — дублирующая проверка спреда)
   if(VolumeConfirmed())
   {
      score++;
      detail += "✅ Vol↑ ";
...
void ManageOpenPositions()
{
   ...
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ...
      {
         double tpDist = MathAbs(curTP - openPrice) / point;
         if(tpDist <= 0.0) tpDist = InpTakeProfit;
...
int OnInit()
{
   ...
   // FIX v4.3 #3: раньше сравнивались часы только когда они совпадали —
   // теперь сравниваются полные ЧЧММ-ключи, что ловит любую конфигурацию,
   // где вход стоит раньше или в момент конца анализа.
   int shortAnalEndKey = InpShortAnalysisHour * 100 + InpShortAnalysisEndMin;
   int shortEntryKey   = InpShortEntryHour * 100 + InpShortEntryMinute;
   if(shortEntryKey <= shortAnalEndKey)
   {
      Print("❌ ERROR: вход SHORT должен быть строго после конца анализа SHORT");
      return INIT_PARAMETERS_INCORRECT;
   }
   int longAnalEndKey = InpLongAnalysisHour * 100 + InpLongAnalysisEndMin;
   int longEntryKey    = InpLongEntryHour * 100 + InpLongEntryMinute;
   ...
   if(h_ma == INVALID_HANDLE || h_adx == INVALID_HANDLE ||
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v4.3 READABLE PANEL запущен");
...
void OnTick()
{
   ...
   int curHM = dt.hour * 100 + dt.min;
   int shortAnalStart = InpShortAnalysisHour * 100 + InpShortAnalysisMinute;
   int shortAnalEnd   = InpShortAnalysisHour * 100 + InpShortAnalysisEndMin;
   int shortEntryFrom = InpShortEntryHour * 100 + InpShortEntryMinute;
   int shortEntryTo   = shortEntryFrom + InpShortEntryWindow;

   int longAnalStart  = InpLongAnalysisHour * 100 + InpLongAnalysisMinute;
   int longAnalEnd    = InpLongAnalysisHour * 100 + InpLongAnalysisEndMin;
   int longEntryFrom  = InpLongEntryHour * 100 + InpLongEntryMinute;
...
bool OpenPosition(int direction)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v4.3 LONG");
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v4.3 SHORT");
...
void DrawPanel()
{
   ...
   int shortAnalStart = InpShortAnalysisHour * 100 + InpShortAnalysisMinute;
   int shortAnalEnd   = InpShortAnalysisHour * 100 + InpShortAnalysisEndMin;
   int shortEntry     = InpShortEntryHour * 100 + InpShortEntryMinute;
   ...
   else if(curHM < shortAnalStart) { shortStatus = "⏳ ждём 03:00"; shortColor = InpColorNeutral; }
   ...
   int longAnalStart = InpLongAnalysisHour * 100 + InpLongAnalysisMinute;
   int longAnalEnd   = InpLongAnalysisHour * 100 + InpLongAnalysisEndMin;
   int longEntry     = InpLongEntryHour * 100 + InpLongEntryMinute;
   ...
   else if(curHM < longAnalStart) { longStatus = "⏳ ждём 15:00"; longColor = InpColorNeutral; }
   ...
   texts[n] = "═══ XAUUSD Reversal v4.3 ═══"; colors[n] = InpColorTitle; n++;
   texts[n] = StringFormat("Время: %02d:%02d", dt.hour, dt.min); colors[n] = InpColorText; n++;
   texts[n] = ""; colors[n] = InpColorText; n++;

   texts[n] = "─── SHORT (03:00→04:00) ───"; colors[n] = InpColorHeader; n++;
   ...
   texts[n] = "─── LONG (15:00→16:00) ───"; colors[n] = InpColorHeader; n++;
