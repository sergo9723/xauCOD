//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v4.6 READABLE PANEL           |
//|                                                                  |
//|  CHANGELOG v4.6 (четвёртый аудит) — найденная проблема:          |
//|                                                                  |
//|  БАГ #E (важно для защиты капитала): состояние стоп-машины —     |
//|     g_consecutive_losses и g_trading_paused — жило только в      |
//|     оперативной памяти EA. Любой перезапуск (перезагрузка VPS,   |
//|     падение/обновление терминала, смена параметров) молча        |
//|     обнулял и счётчик убытков подряд, и саму паузу — защита,     |
//|     задуманная именно на случай "плохой серии", переставала      |
//|     действовать в тот момент, когда она нужнее всего. Открытые   |
//|     позиции эта проблема не касалась (HasOpenShort/Long и так    |
//|     проверяют реальное состояние на брокере, а не память EA) —   |
//|     страдала только пауза после серии убытков.                   |
//|     ИСПРАВЛЕНО: g_consecutive_losses/g_trading_paused теперь     |
//|     сохраняются в терминальные глобальные переменные             |
//|     (GlobalVariableSet/Get) сразу при изменении и восстанав-     |
//|     ливаются в OnInit — пауза переживает перезапуск EA/терминала |
//|     в течение той же торговой недели. При осознанном удалении   |
//|     EA с графика (не рекомпиляция) сохранённое состояние         |
//|     удаляется, чтобы новое подключение не унаследовало паузу     |
//|     от старого запуска.                                          |
//|                                                                  |
//|  Всё из v4.3/v4.4/v4.5 сохранено без изменений: фильтр объёма    |
//|  (исправлен в v4.5), риск % от баланса, безубыток/частичное      |
//|  закрытие/трейлинг, проверка hedging-счёта, минуты-от-полуночи   |
//|  для окон входа, опциональный H1-фильтр тренда, корректный учёт  |
//|  win/loss при частичных закрытиях.                                |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright   "Custom EA v4.6 READABLE PANEL — audited"
#property version     "4.60"
#property strict
#property description "XAUUSD Time EA v4.6 — стоп-машина переживает перезапуск EA/терминала"
...
int OnInit()
{
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v4.6 READABLE PANEL запущен");
...
bool OpenPosition(int direction)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v4.6 LONG");
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v4.6 SHORT");
...
void DrawPanel()
{
   ...
   texts[n] = "═══ XAUUSD Reversal v4.6 ═══"; colors[n] = InpColorTitle; n++;
