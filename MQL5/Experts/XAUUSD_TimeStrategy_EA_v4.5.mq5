//+------------------------------------------------------------------+
//|              XAUUSD_TimeStrategy_EA v4.5 READABLE PANEL           |
//|                                                                  |
//|  CHANGELOG v4.5 (третий аудит, повторная проверка исполнения    |
//|  ордеров, стопов/тейков и точности сигналов) — найденные баги:  |
//|                                                                  |
//|  БАГ #C (КРИТИЧНО, реальная логическая ошибка с v4.3): фильтр   |
//|     объёма VolumeConfirmed() читал массив тиковых объёмов       |
//|     ЗАДОМ НАПЕРЁД. MQL5 при копировании через CopyTickVolume()  |
//|     в динамический массив сам переводит его в таймсерию — index |
//|     0 соответствует САМОМУ СВЕЖЕМУ запрошенному бару, а не      |
//|     самому старому. Код брал vol[need-1] как "текущий" объём —  |
//|     по факту это был САМЫЙ СТАРЫЙ бар из выборки, а "средним"    |
//|     служило среднее по более свежим барам. Фильтр объёма был    |
//|     инвертирован с момента появления в v4.3 — по сути случайный |
//|     (или того хуже) 6-й голос вместо реального подтверждения.   |
//|     ИСПРАВЛЕНО: явный ArraySetAsSeries(vol,true) + vol[0] как    |
//|     текущий бар, среднее — по vol[1..need-1].                    |
//|                                                                  |
//|  БАГ #D (мелкий, defensive-код): в частичном закрытии проверка   |
//|     "TP не задан" делалась как `tpDist<=0`, но если TP реально   |
//|     не задан (curTP=0), MathAbs(0-openPrice)/point даёт ОГРОМНОЕ|
//|     число, а не <=0 — защита не срабатывала. Заменено на явную  |
//|     проверку curTP!=0. В штатном режиме OpenPosition всегда     |
//|     ставит TP, так что это была спящая, а не проявившаяся мина. |
//|                                                                  |
//|  ВАЛИДАЦИЯ: добавлены проверки, которых не хватало — если       |
//|     InpBreakEvenLockPts >= InpBreakEvenTriggerPts, безубыток     |
//|     попытался бы поставить SL ПО ДРУГУЮ СТОРОНУ от текущей цены |
//|     (невалидный ордер на модификацию). Плюс базовые проверки >0 |
//|     для Trigger/Lock/TrailingStart/TrailingStep/PartialCloseAtTP.|
//|                                                                  |
//|  Всё из v4.3/v4.4 сохранено без изменений: риск % от баланса,   |
//|  безубыток/частичное закрытие/трейлинг, стоп-машина после серии |
//|  убытков, проверка hedging-счёта, минуты-от-полуночи для окон   |
//|  входа, опциональный H1-фильтр тренда, корректный учёт win/loss |
//|  при частичных закрытиях.                                        |
//|                                                                  |
...
//+------------------------------------------------------------------+
#property copyright   "Custom EA v4.5 READABLE PANEL — audited"
#property version     "4.50"
#property strict
#property description "XAUUSD Time EA v4.5 — фикс инвертированного фильтра объёма + валидация BE/trailing"
...
int OnInit()
{
   ...
   Print("════════════════════════════════════════════");
   Print("XAUUSD Time EA v4.5 READABLE PANEL запущен");
...
bool OpenPosition(int direction)
{
   ...
   bool ok;
   if(direction == 1)
      ok = trade.Buy(lot, _Symbol, entry, sl, tp, "v4.5 LONG");
   else
      ok = trade.Sell(lot, _Symbol, entry, sl, tp, "v4.5 SHORT");
...
void DrawPanel()
{
   ...
   texts[n] = "═══ XAUUSD Reversal v4.5 ═══"; colors[n] = InpColorTitle; n++;
